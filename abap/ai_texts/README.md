...
| Object | Type | Purpose |
|---|---|---|
| `ZZWN00224895_AI_TEXTS_API` | class | Backend: read/write text pools (`READ/INSERT TEXTPOOL`) and T100 in any installed language, language resolution (ISO ↔ SAP key), length limits, `ENQUEUE_ESRDIRE`, transport recording (`TR_OBJECTS_CHECK` then `TR_OBJECTS_INSERT`). Owns the shared types. |
...
* Program lock via `ENQUEUE_ESRDIRE`. When the object's package is not local, Apply
  records `LIMU REPT` (program texts) or `LIMU MESS` / `R3TR MSAG` (messages) via
  `TR_OBJECTS_CHECK` then `TR_OBJECTS_INSERT`. The insert opens the standard request
  popup if none is known yet; the chosen request is reused for later writes.
